...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    println("Running runHotZoneAnalysis")
    ...
    // joinDf.groupBy("rectangle").count().orderBy("rectangle").show()

    return joinDf.groupBy("rectangle").count().orderBy("rectangle").coalesce(1) // YOU NEED TO CHANGE THIS PART
  }
